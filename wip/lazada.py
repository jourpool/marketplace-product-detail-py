...
#Get itemId from url
value = url.split('/')[-1]
param = value.split('?')[0].split('-')
itemId = param[-2]
sellerId = param[-1]

#Set API url for product information
url_api = 'https://www.lazada.co.id/products/' + itemId + "-" + sellerId 
headers = { 
    'Cache-Control': 'no-cache',
    'Postman-Token': '17878828-6fad-4f71-b59b-51815c406e28',
...
response = requests.get(url_api, headers=headers).content
